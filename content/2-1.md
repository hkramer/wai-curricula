...
* use coding and structure to allow people using only the keyboard to skip over blocks of repeated content
* mark up content so that it is presented in a meaningful sequence
* code distinguishable focus indicators using the CSS `:focus` selector
* implement [or explain] related requirements for designers to create designs for different viewport sizes
* implement [or explain] related requirements for authors to write meaningful page titles
